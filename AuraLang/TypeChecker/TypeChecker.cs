...
public class AuraTypeChecker : IUntypedAuraStmtVisitor<ITypedAuraStatement>, IUntypedAuraExprVisitor<ITypedAuraExpression>
{
	...
	public ITypedAuraStatement Visit(UntypedExpressionStmt exprStmt) =>
		new TypedExpressionStmt(Expression(exprStmt.Expression));
		...
		return WithEnclosingStmt(
			f: () =>
			{
				return InNewScope(() =>
				{
					...
					return new TypedFor(forStmt.For, typedInit, typedCond, typedInc, typedBody, forStmt.ClosingBrace);
				});
			},
			node: forStmt,
			symbolNamespace: ModuleName!
		...
		return WithEnclosingStmt(
			f: () =>
			{
				return InNewScope(() =>
				{
					...
					return new TypedForEach(forEachStmt.ForEach, forEachStmt.EachName, iter, typedBody, forEachStmt.ClosingBrace);
				});
			},
			node: forEachStmt,
			symbolNamespace: ModuleName!
	...
	private TypedLet ShortLetStmt(UntypedLet let)
	{
		...
		var typedShortLet = WithEnclosingStmt(
			f: () =>
			{
				// Type check initializer
				var typedInit = let.Initializer is not null ? Expression(let.Initializer) : null;
				return new TypedLet(null, let.Names, false, let.Mutable, typedInit);
			},
			node: let,
			symbolNamespace: ModuleName!
		);
			...
			f: () =>
			{
				...
				if (implements.Any())
				{
					var valid = implements.Select(impl =>
						...
						.All(b => b);
					if (!valid) throw new MissingInterfaceMethodException(string.Empty, string.Empty, @class.Range); // TODO
				}
		...
		return WithEnclosingStmt(
			f: () =>
			{
				return InNewScope(() =>
				{
					...
					return new TypedWhile(@while.While, typedCond, typedBody, @while.ClosingBrace);
				});
			},
			node: @while,
			symbolNamespace: ModuleName!
	...
	public ITypedAuraStatement Visit(UntypedMultipleImport import)
	{
		...
		return new TypedMultipleImport(import.Import, typedImports, import.ClosingBrace);
	}
	...
	public ITypedAuraStatement Visit(UntypedImport import)
	{
		...
		return new TypedImport(import.Import, import.Package, import.Alias);
	}
	...
	public ITypedAuraStatement Visit(UntypedComment comment) => new TypedComment(comment.Text);
	...
	public ITypedAuraExpression Visit(UntypedAssignment assignment)
	{
		return _enclosingExpressionStore.WithEnclosing(() =>
		{
			...
			var v = FindOrThrow(assignment.Name.Value, ModuleName!, assignment.Range);
			// Ensure that the new value and the variable have the same type
			var typedExpr = ExpressionAndConfirm(assignment.Value, v.Kind);
			return new TypedAssignment(assignment.Name, typedExpr, typedExpr.Typ);
		}, assignment);
	}
	...
	public ITypedAuraExpression Visit(UntypedBinary binary)
	{
		return _enclosingExpressionStore.WithEnclosing(() =>
		{
			...
			return new TypedBinary(typedLeft, binary.Operator, typedRight, typedLeft.Typ);
		}, binary);
	}
	...
	public ITypedAuraExpression Visit(UntypedBlock block)
	{
		return _enclosingExpressionStore.WithEnclosing(() =>
		{
			return InNewScope(() =>
			{
				...
				return new TypedBlock(block.OpeningBrace, typedStmts.ToList(), block.ClosingBrace, blockTyp);
			});
		}, block);
	}
	...
	public ITypedAuraExpression Visit(UntypedCall call)
	{
		return _enclosingExpressionStore.WithEnclosing(() =>
		{
			...
			if (call.Callee is UntypedGet ug)
			{
				...
				{
					var f_ = FindOrThrow(ug.GetName(), im.GetModuleName(), call.Range);
					var funcDeclaration_ = f_.Kind as ICallable;
					// Ensure the function call has the correct number of arguments
					if (funcDeclaration_!.GetParams().Count != call.Arguments.Count + 1) throw new IncorrectNumberOfArgumentsException(call.Arguments.Count, funcDeclaration_.GetParams().Count, call.Range);
			...
			else
			{
				...
				foreach (var arg in funcDeclaration.GetParams())
				{
					...
					var defaultValue = arg.ParamType.DefaultValue ??
									   throw new MustSpecifyValueForArgumentWithoutDefaultValueException(call.GetName(), arg.Name.Value, call.Range);
		...
		return _enclosingExpressionStore.WithEnclosing(() =>
		{
			...
			if (get.Obj is UntypedVariable v && _symbolsTable.GetNamespace(v.Name.Value) is not null)
			{
				...
				if (n is not null)
				{
					...
					var attribute = FindOrThrow(get.Name.Value, symbolNamespace, get.Range);
					return new TypedGet(
			...
			if (objExpr.Typ is not IGettable g) throw new CannotGetFromNonClassException(objExpr.ToString()!, objExpr.Typ, get.GetName(), get.Range);
			...
			var attrTyp = g.Get(get.Name.Value) ?? throw new ClassAttributeDoesNotExistException(objExpr.ToString()!, get.GetName(), get.Range);
			return new TypedGet(objExpr, get.Name, attrTyp);
		}, get);
	}
	...
	public ITypedAuraExpression Visit(UntypedGrouping grouping)
	{
		return _enclosingExpressionStore.WithEnclosing(() =>
		{
			...
			return new TypedGrouping(grouping.OpeningParen, typedExpr, grouping.ClosingParen, typedExpr.Typ);
		}, grouping);
	}
	...
	public ITypedAuraExpression Visit(UntypedIf @if)
	{
		return _enclosingExpressionStore.WithEnclosing(() =>
		{
			...
			return new TypedIf(@if.If, typedCond, typedThen, typedElse, typedThen.Typ);
		}, @if);
	}
	...
	public ITypedAuraExpression Visit<TU>(ListLiteral<TU> literal) where TU : IAuraAstNode
	{
		return _enclosingExpressionStore.WithEnclosing(() =>
		{
			...
			return new ListLiteral<ITypedAuraExpression>(literal.OpeningBracket, typedItems, typedItem.Typ, literal.ClosingBrace);
		}, literal);
	}
	...
	public ITypedAuraExpression Visit(UntypedNil literal) => new TypedNil(literal.Nil);
	...
	public ITypedAuraExpression Visit(UntypedThis @this) => new TypedThis(@this.This, _enclosingClassStore.Peek()!.Typ);
		...
		return _enclosingExpressionStore.WithEnclosing(() =>
		{
			...
			if (unary.Operator.Typ is TokType.Minus)
			{
				if (typedRight.Typ is not AuraInt && typedRight.Typ is not AuraFloat)
					throw new MismatchedUnaryOperatorAndOperandException(unary.Operator.Value, typedRight.Typ, unary.Range);
			}
			...
			return new TypedUnary(unary.Operator, typedRight, typedRight.Typ);
		}, unary);
	}
	...
	public ITypedAuraExpression Visit(UntypedLogical logical)
	{
		return _enclosingExpressionStore.WithEnclosing(() =>
		{
			...
			return new TypedLogical(typedLeft, logical.Operator, typedRight, new AuraBool());
		}, logical);
	}
	...
	private List<Param> TypeCheckParams(List<Param> untypedParams)
	{
		return untypedParams.Select(p =>
		{
			...
			var paramTyp = p.ParamType.Typ is not AuraUnknown u
				? p.ParamType.Typ
				: FindOrThrow(u.Name, ModuleName!, p.Name.Range).Kind;
			return new Param(p.Name, new ParamType(paramTyp, p.ParamType.Variadic, typedDefaultValue));
		}).ToList();
	}
	...
	private TypedCall TypeCheckPositionalParameters(UntypedCall call, ICallable declaration)
	{
		...
		if (declaration.HasVariadicParam() && call.Arguments.Count < declaration.GetParams().Count)
			throw new IncorrectNumberOfArgumentsException(call.Arguments.Count, declaration.GetParams().Count, call.Range);
	...
	private TypedCall TypeCheckNamedParameters(UntypedCall call, ICallable declaration)
	{
		...
		foreach (var missingParam in missingParams)
		{
			...
			var defaultValue = missingParam.ParamType.DefaultValue ??
							   throw new MustSpecifyValueForArgumentWithoutDefaultValueException(call.GetName(), missingParam.Name.Value, call.Range);
	...
	public ITypedAuraStatement Visit(UntypedCheck check)
	{
		...
		// type of `result`
		var enclosingFuncDeclaration = _enclosingFunctionDeclarationStore.Peek() ?? throw new InvalidUseOfCheckKeywordException(check.Range);
		...
		if (enclosingFuncDeclaration.ReturnType?.First() is not AuraResult) throw new InvalidUseOfCheckKeywordException(check.Range);
